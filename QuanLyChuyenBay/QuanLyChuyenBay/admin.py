...
class AirportView(AuthenticatedModelView):
    ...
    column_exclude_list =['sbtg_lich_chuyen_bay']
...
class FlightView(AuthenticatedModelView):
    column_searchable_list = ['ten_chuyen_bay']  # Cho phép tìm kiếm theo tên chuyến bay
    column_filters = ['ten_chuyen_bay']         # Lọc theo tên chuyến bay
    can_view_details = True                     # Cho phép xem chi tiết
    ...
    # Hiển thị tên của sân bay đi và đến
    # column_list = ('ten_chuyen_bay', 'san_bay_di', 'san_bay_den','trang_thai')


    # def get_query(self):
    #     return self.session.query(ChuyenBay).join(ChuyenBay.san_bay_di).join(ChuyenBay.san_bay_den)
    # form_excluded_columns = ('san_bay_di_id','san_bay_den_id')

    #Tùy chỉnh form để chọn sân bay
    # form_args = {
    #     'san_bay_di': {
    #         'query_factory': lambda: SanBay.query.all(),
    #         'get_label': 'ten_san_bay'  # Hiển thị tên sân bay trong form
    #     },
    #     'san_bay_den': {
    #         'query_factory': lambda: SanBay.query.all(),
    #         'get_label': 'ten_san_bay'
    #     }
    # }
